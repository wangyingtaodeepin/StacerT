...
NetworkInfo::NetworkInfo()
{
    ...
    if (lines.count() > 2) {
        defaultNetworkInterface = lines.at(2).split(QRegExp("\\s+")).first();
    } else {
...
{
    quint64 rx = FileUtil::readStringFromFile(rxPath).trimmed().toLong();
...
{
    quint64 tx = FileUtil::readStringFromFile(txPath).trimmed().toLong();
